...
exports.loginAdmin = exports.registerAdmin = exports.loginUser = exports.registerUser = void 0;
...
const registerAdmin = async (req, res) => {
    ...
    connection_1.connection.query("SELECT id FROM admins WHERE email = ?", [email], async (err, result) => {
        if (err)
            return res.status(500).json({ error: err });
        if (result.length > 0) {
        ...
        const hash = await bcrypt_1.default.hash(password, 10);
        connection_1.connection.query("INSERT INTO admins (email, password) VALUES (?, ?)", [email, hash], (err) => {
            if (err)
                return res.status(500).json({ error: err });
...
const loginAdmin = (req, res) => {
    ...
    connection_1.connection.query("SELECT * FROM admins WHERE email = ?", [email], async (err, result) => {
        if (err)
            return res.status(500).json({ error: err });
        if (!result.length) {
        ...
        const token = jsonwebtoken_1.default.sign({ id: admin.id, isAdmin: true }, JWT_SECRET, { expiresIn: "1d" });
        res.json({ token });
    });
};
