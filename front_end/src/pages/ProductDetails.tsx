  ...
  return (
    <div className="bg-[#0f0f13] min-h-screen text-white">
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="mb-10">
          <button onClick={() => navigate("/")} className="flex items-center gap-2 text-zinc-500 hover:text-violet-400 text-sm transition-colors duration-200 mb-6">
            <ArrowLeft size={15} />
            Voltar à loja
          </button>
          <p className="text-xs uppercase tracking-widest text-zinc-600 mb-1">Detalhes do produto</p>
        ...
        <div className="grid md:grid-cols-2 gap-8 bg-[#18181f] border border-[#252530] rounded-2xl overflow-hidden mb-16">
          ...
          <div className="flex flex-col justify-center p-10">
            <h2 className="text-3xl font-extrabold tracking-tight text-white leading-tight mb-3">{product.name}</h2>
            ...
            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border w-fit mb-8 ${product.stock > 5 ? "bg-emerald-400/10 text-emerald-400 border-emerald-400/20" : "bg-amber-400/10 text-amber-400 border-amber-400/20"}`}>
              <Layers size={11} />
              {product.stock > 5 ? `${product.stock} unidades disponíveis` : `Apenas ${product.stock} restantes`}
            </span>
            <button
              onClick={() => { addToCart(product); toast.success("Produto adicionado ao carrinho 🛒"); }}
        ...
        <section>
          <div className="mb-8">
            <p className="text-xs uppercase tracking-widest text-zinc-600 mb-1">Explore mais</p>
            <h2 className="text-3xl font-extrabold tracking-tight text-white">
              Outros <span className="text-violet-400">Produtos</span>
            </h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {otherProducts.filter((p) => p.id !== product.id).slice(0, 4).map((item) => (
              <div key={item.id} className="group bg-[#18181f] border border-[#252530] rounded-2xl overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:border-[#3d3d55]">
                <div onClick={() => navigate(`/product/${item.id}`)} className="relative h-44 bg-[#111118] flex items-center justify-center cursor-pointer overflow-hidden">
                  <img
                ...
                <div className="p-4 flex flex-col gap-3 flex-1">
                  <div>
                    <h3 className="text-[#f0f0f5] font-semibold text-sm leading-snug truncate">{item.name}</h3>
                    <p className="text-violet-400 font-bold text-base mt-1">R$ {Number(item.price).toFixed(2)}</p>
                  </div>
                  <button
                    onClick={() => { addToCart(item); toast.success(`${item.name} adicionado 🛒`); }}
